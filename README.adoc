...
* `src/main/resources`:
** `server.cfg`: configuration file of embedded server for web page and REST API
** `marian-translator.cfg`: Okapi connector configuration file for Marian NMT, including connection settings for pre-/postprocessing server
...
** `defaultSegmentation.srx`: configuration file of the Okapi segmentation step
** `de/dfki/mlt/transins/*Configuration.yml`: configuration files used by the Okapi HTML filter
